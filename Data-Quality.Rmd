...
setwd(path)
mortality <- read_parquet("mort2015.parquet")
write_csv(head(mortality), "first_few_lines.csv")
...
m1 <- lm(num_extra_factors ~ marstat + ager52, data = mort)

library(MASS)

full_mod  <- lm(num_extra_factors ~ marstat + ager52 + ucr39, data = mort)
