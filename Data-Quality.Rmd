...
parquet_dir    <- "/Users/amymann/Documents/Data Quality Project/data/parquet"
detail_csv     <- "/Users/amymann/Documents/Data Quality Project/data_quality/accident-detail-codes.csv"
years_wanted   <- 1999:2023
secondary_cols <- paste0("record_", 1:20)                 # add econd*/econds* if needed

extra_cols     <- c("year", "sex", "race5", "educ2003", "marstat", "placdth")
clean_icd <- function(x) {
...
ds0      <- open_dataset(parquet_dir, unify_schemas = FALSE)  # cheap scan
sch_base <- ds0$schema
...
my_schema <- stringify(sch_base, sec_cols_on_disk)
...
# ── 0 · Helper (same cleaner you used in the pipeline) ----------------------
clean_icd <- function(x) {
  x |>
    toupper() |>
...
# ── 1 · Grab the 519 067-th row *by position* ------------------------------
row_idx <- 519067                        # ← the index you care about

row_one <- accident_flagged %>%          # the final tibble you already built
  slice(row_idx)                         # pick that row by position

if (nrow(row_one) == 0) {
  stop("Row ", row_idx, " does not exist in accident_flagged.")
}

# ── 2 · Pivot its secondary codes into long form ---------------------------
sec_one <- row_one %>%
  pivot_longer(starts_with("record_"),      # record_1 … record_20
               names_to  = "slot",
               values_to = "detail") %>%
  mutate(
    ucod   = clean_icd(ucod),
    detail = clean_icd(detail)
  ) %>%
  filter(detail != "", detail != ucod)      # drop blanks + self-matches

# ── 3 · See what hit the dictionary ----------------------------------------
matches <- sec_one %>%
  inner_join(detail_dict, by = c("ucod", "detail"))

cat("Row index:", row_idx,
    "\nCertificate UCOD:", row_one$ucod,
    "\nHas_detail flag:", row_one$has_detail, "\n\n")

if (nrow(matches) == 0) {
  cat("→   No secondary code in this certificate appears in the dictionary.\n",
      "    The flag should be **N**.  If it shows **Y**, check your\n",
      "    joins or earlier mutate steps for an unintended self-match.\n")
} else {
  cat("→   The following secondary code(s) triggered **Y**:\n\n")
  print(matches %>% select(slot, detail), row.names = FALSE)
}

cat("\nFull list of secondary codes in this certificate:\n")
print(sec_one %>% select(slot, detail), row.names = FALSE)
