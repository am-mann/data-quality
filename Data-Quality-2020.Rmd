...
make_features_fast <- function(df){
  setDT(df)                             # convert to data.table in-place
  df[, rid := .I]                       # helper row id

  # explode long
  long <- melt(df,
               id.vars      = c("rid","uc3","age_grp","sex_male"),
               measure.vars = patterns("^record_"),
               value.name   = "cc")[!is.na(cc) & cc != ""]

  long[, cc := clean_icd3(cc)]
  long <- unique(long[cc != ""])        # drop empties & dupes

  # cast wide: one sparse indicator column per contributing cause
  wide <- dcast(long,
                rid + uc3 + age_grp + sex_male ~ cc,
                fun.aggregate = length,
                value.var     = "cc",
                drop          = FALSE)

  wide[, rid := NULL][]                 # drop helper and return
}

train_feat  <- make_features_fast(train, bin_map)
...
train_tidy <- train_feat |>
  select(uc3, age_grp, sex_male, all_of(keep_cc)) |>
  mutate(
    age_grp  = factor(age_grp, levels = levels(mort_df$age_grp)),
...
```
Calculate DQ score using linear model with age/sex for top ten garbage codes
```{r}
# ──────────────────────────────────────────────────────────────
# Data-Quality Project · Garbage-Code DQ score
# ORIGINAL WORKING VERSION – 2025-07-07
# ──────────────────────────────────────────────────────────────

## 0 · Setup – packages, paths, helpers  ← unchanged
required <- c("arrow", "dplyr", "tidyr", "stringr", "purrr",
              "nnet", "matrixStats", "glue", "readr")
missing  <- setdiff(required, rownames(installed.packages()))
if (length(missing)) install.packages(missing)
...
compute_dq <- function(garbage_code, candidate_codes) {
  ...
  ## fill any cc_ columns that were present in training but absent here
  ...
  for (i in seq_len(nrow(strata))) {
    ...
    fit_sub <- nnet::multinom(uc3 ~ .,
                              data = tr,
                              MaxNWts = 47128,
                              trace   = FALSE)

    probs_B <- predict(fit_sub, newdata = te, type = "probs")

    ## multinom() returns a vector instead of a matrix if k = 2
    if (is.null(dim(probs_B))) {
      lev <- fit_sub$lev          # NB: *sub*, not fit_A
      probs_B <- cbind(
        setNames(1 - probs_B, lev[1]),
        setNames(    probs_B, lev[2])
      )
    }

    H_B[te_idx] <- apply(probs_B, 1, entropy_bits)
  }
...
```
```{r}
candidate_codes <- c("C54","C53")          # whatever the two targets are
train <- mort %>% filter(uc3 %in% cand)
garbage_code <- "C55"
...
feature_cols <- setdiff(names(train_tidy), c("uc3", "age_grp", "sex_male"))
test_feat <- test_feat %>%
  select(age_grp, sex_male, all_of(intersect(feature_cols, names(.)))) %>%
  { 
    missing <- setdiff(feature_cols, names(.))
    if (length(missing)) .[missing] <- 0L
    .
  } %>%
  select(age_grp, sex_male, all_of(feature_cols))
  test_feat <- test_feat %>% select(-starts_with("record_"))

  ## 3 · MODEL A  (no age/sex)
  set.seed(123)
  fit_A <- nnet::multinom(uc3 ~ ., data = select(train_tidy, -age_grp, -sex_male),
                          MaxNWts = 10500, trace = FALSE)
 probs_A <- predict(fit_A, newdata = select(test_feat, -age_grp, -sex_male),
                     type = "probs")
 

 
print(str(probs_A))             # vector vs. matrix?
  H_A  <- apply(probs_A, 1, entropy_bits)
  DQ_A <- 1 - mean(H_A) / log2(length(candidate_codes))

  ## 4 · MODEL B  (stratified by age/sex)
  strata <- count(train_tidy, age_grp, sex_male) |> filter(n >= 100)
  H_B <- rep(NA_real_, nrow(test_feat))

  for (i in seq_len(nrow(strata))){
    ag <- strata$age_grp[i]; sm <- strata$sex_male[i]
    tr <- train_tidy |> filter(age_grp == ag, sex_male == sm) |> select(-age_grp, -sex_male)
    te_idx <- which(test_feat$age_grp == ag & test_feat$sex_male == sm)
    te <- test_feat[te_idx, ] |> select(-age_grp, -sex_male)
    if (nrow(tr) < 50 || nrow(te) < 10) next
    fit_sub <- nnet::multinom(uc3 ~ ., data = tr, MaxNWts = 10500, trace = FALSE)
    H_B[te_idx] <- apply(predict(fit_sub, newdata = te, type = "probs"),
                         1, entropy_bits)
  }
  DQ_B <- 1 - mean(H_B, na.rm = TRUE) / log2(length(candidate_codes))
