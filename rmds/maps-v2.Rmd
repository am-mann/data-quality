...
p_ri_a <- ggplot() +
  ...
  labs(title = "Re-assignment Index, 1999–2005", fill = NULL) +
...
p_ri_b <- ggplot() +
  ...
  make_red_scale(ri_lims) +
  coord_sf(xlim = base_extent$xlim, ylim = base_extent$ylim, expand = FALSE) +
  labs(title = "Re-assignment Index, 2020–2022", fill = NULL) +
  theme_void(base_size = 11) + theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
p_lod_a <- ggplot() +
  ...
  coord_sf(xlim = base_extent$xlim, ylim = base_extent$ylim, expand = FALSE) +
  labs(title = "Level of detail, 1999–2005", fill = NULL) +
  theme_void(base_size = 11) + theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
p_lod_b <- ggplot() +
  ...
  make_red_scale(lod_lims) +
  coord_sf(xlim = base_extent$xlim, ylim = base_extent$ylim, expand = FALSE) +
  labs(title = "Level of detail, 2020–2022", fill = NULL) +
  theme_void(base_size = 11) + theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
p1 <- ggplot() +
  ...
  labs(title = "1999–2005", fill = NULL) + theme_void(base_size = 11) +
...
p2 <- ggplot() +
  ...
  geom_sf(data = states_tf, fill = NA, colour = "white", linewidth = 0.25) +
  make_scale(pg_lims) + coord_sf(xlim = base_extent$xlim, ylim = base_extent$ylim, expand = FALSE) +
  labs(title = "2013–2019", fill = NULL) + theme_void(base_size = 11) +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
p3 <- ggplot() +
  ...
  geom_sf(data = states_tf, fill = NA, colour = "white", linewidth = 0.25) +
  make_scale(pg_lims) + coord_sf(xlim = base_extent$xlim, ylim = base_extent$ylim, expand = FALSE) +
  labs(title = "2020–2022", fill = NULL) + theme_void(base_size = 11) +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
# ---------------------------
# 3) Correlation plan with inference (county-level, period-averaged or overall)
# ---------------------------
# We do this on county averages across the full study window (1999–2022) by default.
df_county_avg <- df0 %>%
  group_by(county_ihme) %>%
  summarise(
    across(all_of(metric_cols), ~ mean(suppressWarnings(as.numeric(.x)), na.rm = TRUE)),
...
# Bootstrap CI for correlation (works for Pearson and Spearman)
boot_cor_ci <- function(x, y, method = c("spearman","pearson"), B = 500L) {
  ...
  rs <- replicate(B, {
    ...
    suppressWarnings(stats::cor(x[ii], y[ii], method = method))
  })
  stats::quantile(rs, probs = c(0.025, 0.975), na.rm = TRUE, names = FALSE) %>%
    setNames(c("lo","hi"))
}

# Only run if we actually have covariates to correlate against
corr_tbl <- tibble()
if (length(covar_cols) > 0) {
  corr_tbl <- expand_grid(
    ...
    mutate(
      x = map2(metric, df_county_avg, ~ df_county_avg[[.x]]),
      y = map2(covariate, df_county_avg, ~ df_county_avg[[.x]]),
      n = map2_int(x, y, ~ sum(is.finite(.x) & is.finite(.y))),
      r = map3_dbl(x, y, method, ~ suppressWarnings(stats::cor(.x, .y, method = ..3, use = "pairwise.complete.obs"))),
      p_value = pmap_dbl(list(x, y, method), function(x, y, method) {
        ok <- is.finite(x) & is.finite(y)
        if (sum(ok) < 25) return(NA_real_)
        suppressWarnings(stats::cor.test(x[ok], y[ok], method = method)$p.value)
      }),
      ci = pmap(list(x, y, method), function(x, y, method) boot_cor_ci(x, y, method = method, B = 500L)),
      r_lo = map_dbl(ci, ~ .x["lo"]),
      r_hi = map_dbl(ci, ~ .x["hi"])
    ) %>%
    select(metric, covariate, method, n, r, r_lo, r_hi, p_value) %>%
    group_by(method) %>%
    mutate(p_adj_fdr = p.adjust(p_value, method = "BH")) %>%
    ungroup()
  ...
  message("[STATS] No continuous covariates found for correlation tests; skipping correlation inference.")
}
```
