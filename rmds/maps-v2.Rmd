...
NEW: Make a new plot with RI maps, garbage codes, and level of detail for new paper
```{r}
# ──────────────────────────────────────────────────────────────
...
# NOTE: For Brewer "Reds", default direction already gives higher = darker
red_scale <- function(lims) {
...
make_map <- function(sf_obj, fill_var, title, lims) {
  ggplot2::ggplot() +
    ggplot2::geom_sf(data = sf_obj, ggplot2::aes(fill = {{ fill_var }}), colour = NA) +
    ggplot2::geom_sf(data = states, fill = NA, colour = "white", linewidth = 0.25) +
    red_scale(lims) +
    ggplot2::coord_sf(xlim = base_extent$xlim, ylim = base_extent$ylim, expand = FALSE) +
...
make_pg_map <- function(sf_obj, title, lims) {
  ggplot2::ggplot() +
    ggplot2::geom_sf(data = sf_obj, ggplot2::aes(fill = prop_garbage), colour = NA) +
    ggplot2::geom_sf(data = states, fill = NA, colour = "white", linewidth = 0.25) +
    red_scale(lims) +
    ggplot2::coord_sf(xlim = base_extent$xlim, ylim = base_extent$ylim, expand = FALSE) +
    ggplot2::labs(title = title, fill = NULL) +
...
ts_all <- bind_rows(ts_pg, ts_ri, ts_det, ts_agg)

# Long format: one row per metric × year × predictor
ts_all_out_long <- ts_all %>%
  arrange(metric, year, predictor)

ts_all_long_file <- here::here("figures", "correlations_time_series_long.csv")
readr::write_csv(ts_all_out_long, ts_all_long_file)
message("Wrote time-series (long) CSV: ", ts_all_long_file)

# Wide format: one row per metric × year, predictors as columns
ts_all_out_wide <- ts_all %>%
  select(metric, year, predictor, corr) %>%
  tidyr::pivot_wider(
...
ts_all_wide_file <- here::here("figures", "correlations_time_series_wide.csv")
readr::write_csv(ts_all_out_wide, ts_all_wide_file)
message("Wrote time-series (wide) CSV: ", ts_all_wide_file)
```
...
Statistical analysis chunk (DEC 21)
```{r}
# ============================================================
# NEW: Statistical inference for (1) before/after changes and
#      (2) associations with county covariates
# ============================================================
# Implements Matt's suggestion:
#  - Paired Wilcoxon signed-rank tests on county-level BEFORE vs AFTER period means
#  - Correlation tests (Spearman + Pearson) between county-level metrics and covariates
# Outputs:
#  - output/stats_prepost_signedrank_county.csv
#  - output/stats_correlations_county.csv
# ============================================================
...
# ---------------------------
# 0) Identify the analysis frame to use
# ---------------------------
# We prefer `base_sel` because it typically already has:
# county_ihme, year, prop_garbage, ri, detail_icd4, (maybe agg_index/direction_score),
# and often covariates like income_raw, ph_pc_med, reporting_type.
if (!exists("base_sel")) {
  stop("This chunk expects `base_sel` to exist (built earlier). ",
       "If your main county-year table has a different name, replace `base_sel` below.")
}
...
# ---------------------------
# 1) Define periods and compute county-level period means
# ---------------------------
period_of_year <- function(y) {
  dplyr::case_when(
    y %in% 1999:2005 ~ "1999_2005",
...
df0 <- df0 %>% mutate(period = period_of_year(year)) %>% filter(!is.na(period))

# Candidate metric columns (use what exists)
metric_candidates <- c(
  "prop_garbage", "ri", "ri_post_only", "detail_icd4",
  "agg_index", "direction_score"
)
metric_cols <- intersect(metric_candidates, names(df0))
if (length(metric_cols) == 0) {
  stop("No metric columns found in `base_sel`. Looked for: ",
       paste(metric_candidates, collapse = ", "))
}

# Pick continuous covariates if available
covar_candidates <- c(
  "income_raw", "income_med", "avg_income",
  "ph_pc_med", "ph_pc", "ph_pc_raw",
  "excess_covid", "excess_covid_deaths", "excess_deaths", "covid_excess"
)
covar_cols <- intersect(covar_candidates, names(df0))

# County-level period means (metrics + covariates)
df_period_means <- df0 %>%
  group_by(county_ihme, period) %>%
  summarise(
    across(all_of(metric_cols), ~ mean(suppressWarnings(as.numeric(.x)), na.rm = TRUE)),
    across(all_of(covar_cols),  ~ mean(suppressWarnings(as.numeric(.x)), na.rm = TRUE)),
    .groups = "drop"
  )

# ---------------------------
# 2) Paired before/after tests (Wilcoxon signed-rank on paired county period means)
# ---------------------------
run_signedrank_pair <- function(dat_long, metric, pre_period, post_period, label) {
  dat_wide <- dat_long %>%
    select(county_ihme, period, value = all_of(metric)) %>%
    filter(period %in% c(pre_period, post_period)) %>%
    pivot_wider(names_from = period, values_from = value)

  pre  <- dat_wide[[pre_period]]
  post <- dat_wide[[post_period]]
  ok <- is.finite(pre) & is.finite(post)
  pre <- pre[ok]; post <- post[ok]
  diffs <- post - pre
  ...
  # One-sample signed-rank on differences -> tests median shift != 0
  wt <- suppressWarnings(stats::wilcox.test(diffs, mu = 0, alternative = "two.sided",
                                           conf.int = TRUE, conf.level = 0.95, exact = FALSE))
  ci <- wt$conf.int
  est <- unname(wt$estimate)  # Hodges–Lehmann (pseudo-median) estimate
  ...
  tibble(
    ...
    q25_diff = quantile(diffs, 0.25, na.rm = TRUE),
    q75_diff = quantile(diffs, 0.75, na.rm = TRUE),
...
# Define the two “before/after” contrasts that usually map cleanly to manuscript language
comparisons <- tribble(
  ~pre,        ~post,       ~label,
...
signedrank_tbl <- expand_grid(
  metric = metric_cols,
  comparisons
) %>%
  mutate(
    out = pmap(
      list(pre, post, label, metric),
      ~ run_signedrank_pair(df_period_means, metric = ..4, pre_period = ..1, post_period = ..2, label = ..3)
    )
  ) %>%
  select(out) %>%
  unnest(out) %>%
...
out_signedrank <- here("output", "stats_prepost_signedrank_county.csv")
write_csv(signedrank_tbl, out_signedrank)
message("[STATS] Wrote paired signed-rank results: ", out_signedrank)
print(signedrank_tbl)

# ---------------------------
# 3) Correlation plan with inference (county-level averages)
# ---------------------------
...
df_county_avg <- df0 %>%
  ...
  summarise(
    ...
    across(all_of(covar_cols),  ~ mean(as.numeric(.x), na.rm = TRUE)),
    .groups = "drop"
  )

boot_cor_ci <- function(x, y, method = c("spearman","pearson"), B = 500L) {
  method <- match.arg(method)
  ok <- is.finite(x) & is.finite(y)
  x <- x[ok]; y <- y[ok]
  n <- length(x)
  if (n < 25) return(c(lo = NA_real_, hi = NA_real_))
  rs <- replicate(B, {
    ii <- sample.int(n, size = n, replace = TRUE)
    suppressWarnings(cor(x[ii], y[ii], method = method))
  })
  quantile(rs, c(0.025, 0.975), na.rm = TRUE)
}

corr_tbl <- expand_grid(
  metric = metric_cols,
  ...
  mutate(
    out = pmap(
      ...
      function(metric, covariate, method) {

        x <- df_county_avg[[metric]]
        y <- df_county_avg[[covariate]]
        ok <- is.finite(x) & is.finite(y)
        ...
        r_val <- suppressWarnings(cor(x, y, method = method, use = "pairwise.complete.obs"))
        p_val <- suppressWarnings(cor.test(x[ok], y[ok], method = method)$p.value)
        ci <- boot_cor_ci(x, y, method = method, B = 500L)
...
out_corr <- here("output", "stats_correlations_county.csv")
write_csv(corr_tbl, out_corr)

message("[STATS] Wrote correlation inference table: ", out_corr)
print(corr_tbl)
...
```{r}
# =====================================================================
# Stand-alone DIAGNOSTICS + SENSITIVITY + TIME-SERIES PLOTS CHUNK
# - Raw covariate plots (income, per-capita public health exp, excess mortality, etc.)
# - Sensitivity: stability of "level of detail" vs deaths (why 1,500)
# - Time series of indices: median, IQR, middle 95% over years
#
# Assumes you have already produced:
#   data/county_year_quality_metrics.csv.gz   (from your pipeline)
# and (optionally) county-level covariate files somewhere under data/ or data_raw/.
# =====================================================================
...
fig_dir <- here("figures", "dq_diagnostics")
dir.create(fig_dir, recursive = TRUE, showWarnings = FALSE)

set.seed(1)

# Which columns in your county-year file correspond to the 3 dimensions?
# (Adjust if your helper outputs differ.)
metric_cols <- c(
  prop_garbage = "prop_garbage",
  RI           = "DQ_overall",          # <-- change if your RI column has a different name
  detail       = "DQ_expH_over_K"       # <-- change if your level-of-detail column differs
)

# helper: robust quantiles
q_ <- function(x, p) as.numeric(stats::quantile(x, probs = p, na.rm = TRUE, names = FALSE, type = 7))

# -----------------------------
# load county-year DQ metrics
# -----------------------------
dq <- readr::read_csv(dq_path, show_col_types = FALSE)

# sanity: ensure year + county exist
stopifnot("year" %in% names(dq))
stopifnot(county_var %in% names(dq))

# enforce numeric year
dq <- dq %>% mutate(year = as.integer(year))

# -----------------------------
# OPTIONAL: load & merge covariates (best-effort auto-detect)
# -----------------------------
# This tries several common file names/locations; if none exist, we still produce the DQ plots.
cov_candidates <- c(
  here("data", "county_covariates.csv"),
  here("data", "county_covariates.csv.gz"),
  here("data_raw", "county_covariates.csv"),
  here("data_raw", "county_covariates.csv.gz"),
  here("data", "covariates", "county_covariates.csv"),
  here("data", "covariates", "county_covariates.csv.gz")
)
cov_path <- cov_candidates[file.exists(cov_candidates)][1]

cov <- NULL
if (!is.na(cov_path) && nzchar(cov_path)) {
  message("Reading covariates: ", cov_path)
  cov <- readr::read_csv(cov_path, show_col_types = FALSE)

  # Try to standardize key columns (best-effort)
  if (!county_var %in% names(cov)) {
    # common alternatives
    if ("county" %in% names(cov)) cov <- cov %>% rename(!!county_var := county)
    if ("fips" %in% names(cov) && !county_var %in% names(cov)) cov <- cov %>% rename(!!county_var := fips)
  }
  if (!"year" %in% names(cov)) {
    # allow period-only covariates; we won't merge by year then
    message("Covariates have no 'year' column; will plot as cross-sectional (no merge).")
  } else {
    cov <- cov %>% mutate(year = as.integer(year))
  }

  # Merge if keys exist
  if (all(c(county_var, "year") %in% names(cov))) {
    dq_cov <- dq %>% left_join(cov, by = c(county_var, "year"))
  } else if (county_var %in% names(cov)) {
    dq_cov <- dq %>% left_join(cov, by = county_var)
  } else {
    message("Covariates missing a join key; skipping merge.")
    dq_cov <- dq
    cov <- NULL
  }
} else {
  message("No covariates file found (county_covariates.csv*). Will still plot DQ indices over time + sensitivity.")
  dq_cov <- dq
}

# -----------------------------
# 1) Raw covariate plots
# -----------------------------
# We will look for plausible covariate column names and plot their raw distributions over time.
# Edit/extend this list to match your actual covariate column names.
covariate_name_map <- list(
  income = c("income", "mean_income", "avg_income", "median_income", "percap_income"),
  public_health_exp_pc = c("ph_exp_pc", "public_health_exp_pc", "percap_public_health_spending",
                           "per_capita_public_health_expenditure", "public_health_spending_pc"),
  health_exp_pc = c("health_exp_pc", "percap_health_expenditure", "per_capita_health_expenditure"),
  excess_covid_rate = c("excess_covid_rate", "excess_mortality_rate", "covid_excess_mortality_rate",
                        "excess_natural_rate", "excess_rate")
)

find_first_existing <- function(cands, df) {
  hit <- cands[cands %in% names(df)][1]
  if (is.na(hit) || !nzchar(hit)) NA_character_ else hit
}

cov_hits <- purrr::imap_chr(covariate_name_map, ~ find_first_existing(.x, dq_cov))
cov_hits <- cov_hits[!is.na(cov_hits)]

if (length(cov_hits) > 0) {
  cov_long <- dq_cov %>%
    select(any_of(c(county_var, "year", cov_hits))) %>%
    pivot_longer(cols = all_of(cov_hits), names_to = "covariate", values_to = "value") %>%
    mutate(covariate = factor(covariate, levels = cov_hits))

  # (a) Raw time series distribution: median + IQR + 2.5/97.5
  cov_summ <- cov_long %>%
  ...
  p_cov_ts <- ggplot(cov_summ, aes(x = year, y = med)) +
    ...
    labs(
      title = "Raw covariates over time (median, IQR, middle 95%)",
      x = NULL, y = NULL
    ) +
    theme_minimal(base_size = 12)

  ggsave(file.path(fig_dir, "covariates_raw_timeseries.png"), p_cov_ts, width = 9, height = 10, dpi = 300)

  # (b) Cross-sectional histograms (pooled across years)
  p_cov_hist <- ggplot(cov_long, aes(x = value)) +
    geom_histogram(bins = 60) +
    facet_wrap(~ covariate, scales = "free", ncol = 1) +
    labs(
      title = "Raw covariate distributions (all county-years pooled)",
      x = NULL, y = "Count"
    ) +
    theme_minimal(base_size = 12)

  ggsave(file.path(fig_dir, "covariates_raw_hist.png"), p_cov_hist, width = 9, height = 10, dpi = 300)

} else {
  message("No recognized covariate columns found in dq_cov. Add your covariate column names to covariate_name_map.")
}

# -----------------------------
# 2) Sensitivity: stability of level-of-detail vs number of deaths
# -----------------------------
# Goal: show why 1,500 is a reasonable threshold by plotting how much the detail metric
# varies when you (a) repeatedly subsample within large counties/county-sets at different n
# and (b) compute the detail metric on each subsample.
#
# IMPORTANT:
# This chunk will try to use your helper(s) if present; otherwise it will fall back to a proxy:
# Shannon entropy of uc4 among non-garbage deaths (still useful to show stabilization vs n).
#
# For the "true" standardized detail you described in the paper, you likely have helper functions
# inside dq_entropy_helper.R. If you do, add a small wrapper `compute_detail_from_sample(ds)` below.
# -----------------------------

# (A) Identify a set of high-death county-years to subsample from
if (!"n_cert" %in% names(dq)) {
  warning("dq file does not have n_cert; sensitivity plot will be skipped. (You can add n_cert to output.)")
} else {

  # Choose donor county-years with lots of deaths (so subsampling makes sense)
  donors <- dq %>%
    filter(!is.na(n_cert), n_cert >= 20000) %>%
    select(all_of(c(county_var, "year", "n_cert"))) %>%
    slice_sample(n = min(20, n()))

  if (nrow(donors) == 0) {
    message("No donor county-years with n_cert >= 20000 found; lower the donor threshold if needed.")
  } else {

    # --- You MUST be able to read the underlying microdata for this part.
    # We will attempt to infer parquet directory; edit if your path differs.
    ...
    if (is.na(parquet_dir)) {
      message("Could not locate parquet_dir (data_private/mcod*). Skipping sensitivity subsampling.")
    } else {
      suppressPackageStartupMessages(library(arrow))

      # read-one-year helper (minimal cols)
      read_year_min <- function(y) {
        f <- file.path(parquet_dir, paste0("mort", y, ".parquet"))
        stopifnot(file.exists(f))
        arrow::read_parquet(f, as_data_frame = TRUE,
                           col_select = intersect(c("ucod", county_var, "year"), names(arrow::read_parquet(f, as_data_frame = FALSE))))
      }

      # proxy detail metric from a sample (fallback)
      canonical_icd <- function(x) stringr::str_remove_all(stringr::str_to_upper(x), "[^A-Z0-9\\.]")
      uc4_of <- function(ucod) substr(canonical_icd(ucod), 1, 4)

      compute_detail_from_sample <- function(df_sample) {
        # If you have a true standardized detail function, replace this with it.
        # Fallback: scaled entropy of uc4 among all deaths in the sample.
        u <- uc4_of(df_sample$ucod)
        u <- u[!is.na(u) & nzchar(u)]
        if (length(u) == 0) return(NA_real_)
        p <- table(u) / length(u)
        H <- -sum(as.numeric(p) * log(as.numeric(p)))
        K <- length(p)
        if (K <= 1) return(0)
        100 * (H / log(K))
      }

      # Subsample sizes (include 1500 explicitly)
      ns <- c(250, 500, 750, 1000, 1250, 1500, 2000, 3000, 5000, 10000)
      B  <- 200  # replicates per n per donor (adjust if heavy)

      # Pull microdata for donor years only
      donor_years <- sort(unique(donors$year))
      micro_by_year <- setNames(vector("list", length(donor_years)), donor_years)

      for (y in donor_years) {
        message("Loading microdata for sensitivity: year ", y)
        micro_by_year[[as.character(y)]] <- read_year_min(y) %>%
          mutate(year = as.integer(year))
      }

      # Create sensitivity draws
      sens <- purrr::pmap_dfr(
        donors,
        function(...) {
          row <- tibble::tibble(...)
          y <- row$year
          cty <- row[[county_var]]
          dfy <- micro_by_year[[as.character(y)]]

          pool <- dfy %>% filter(.data[[county_var]] == cty)
          if (nrow(pool) < max(ns)) {
            # skip if not enough in that county-year
            return(tibble())
          }

          purrr::map_dfr(ns, function(n_) {
            reps <- purrr::map_dbl(seq_len(B), function(b) {
              samp <- pool %>% slice_sample(n = n_, replace = FALSE)
              compute_detail_from_sample(samp)
            })
            tibble(
              year = y,
              !!county_var := cty,
              n = n_,
              detail = reps,
              sd = sd(reps, na.rm = TRUE),
              iqr = IQR(reps, na.rm = TRUE),
              med = median(reps, na.rm = TRUE)
            )
          })
        }
      )
      ...
      if (nrow(sens) == 0) {
        message("Sensitivity draws returned 0 rows (likely donors didn't have enough microdata).")
      } else {
        # Summarise across donors: how variability shrinks with n
        sens_summ <- sens %>%
          group_by(n) %>%
          summarise(
            med_sd = median(sd, na.rm = TRUE),
            med_iqr = median(iqr, na.rm = TRUE),
            .groups = "drop"
          )

        p_sens <- ggplot(sens_summ, aes(x = n, y = med_sd)) +
          geom_line() +
          geom_point() +
          geom_vline(xintercept = 1500, linetype = 2) +
          scale_x_continuous(trans = "log10", breaks = ns, labels = ns) +
          labs(
            title = "Sensitivity: stability of level-of-detail vs sample size",
            subtitle = "Median within-donor SD across repeated subsamples; dashed line at n = 1,500",
            x = "Sample size (log10 scale)",
            y = "Median SD of detail across replicates"
          ) +
          theme_minimal(base_size = 12)

        ggsave(file.path(fig_dir, "detail_sensitivity_sd_vs_n.png"), p_sens, width = 9, height = 5, dpi = 300)

        # Optional: show the distribution of replicate detail at each n (pooled)
        p_sens_box <- ggplot(sens, aes(x = factor(n), y = detail)) +
          geom_boxplot(outlier.alpha = 0.2) +
          geom_vline(xintercept = which(ns == 1500), linetype = 2) +
          labs(
            title = "Sensitivity: replicate detail distributions by sample size",
            x = "Sample size (n)",
            y = "Detail (replicate estimates)"
          ) +
          theme_minimal(base_size = 12) +
          theme(axis.text.x = element_text(angle = 45, hjust = 1))

        ggsave(file.path(fig_dir, "detail_sensitivity_box_by_n.png"), p_sens_box, width = 10, height = 6, dpi = 300)
      }
    }
  }
}
...
# -----------------------------
# 3) Indices over time: median, IQR, middle 95%
# -----------------------------
# Build a long table for the 3 core indices, then summarise per year.
missing_metrics <- names(metric_cols)[!unlist(metric_cols) %in% names(dq)]
if (length(missing_metrics) > 0) {
  message(
    "These metric columns were not found and will be skipped:\n",
    paste0("  - ", missing_metrics, " (expected column: ", metric_cols[missing_metrics], ")", collapse = "\n")
  )
}

metric_cols_present <- metric_cols[metric_cols %in% names(dq)]
if (length(metric_cols_present) == 0) {
  stop("None of the metric columns in metric_cols were found in dq. Update metric_cols to match your file.")
}

dq_long <- dq %>%
  select(year, all_of(unname(metric_cols_present))) %>%
  pivot_longer(cols = -year, names_to = "metric", values_to = "value") %>%
  mutate(metric = factor(metric, levels = unname(metric_cols_present)))
...
metric_pretty <- c(
  prop_garbage = "Proportion garbage-coded (raw)",
  DQ_overall   = "Re-assignability Index (RI)",
  DQ_expH_over_K = "Level of detail"
)

p_dq_ts <- ggplot(dq_ts, aes(x = year, y = med)) +
  geom_ribbon(aes(ymin = lo95, ymax = hi95), alpha = 0.15) +
  geom_ribbon(aes(ymin = q25, ymax = q75), alpha = 0.25) +
  geom_line() +
  facet_wrap(~ metric, scales = "free_y", ncol = 1,
             labeller = as_labeller(metric_pretty, default = label_value)) +
  labs(
    title = "Data quality indices over time (median, IQR, middle 95%)",
    x = NULL, y = NULL
  ) +
  theme_minimal(base_size = 12)

ggsave(file.path(fig_dir, "dq_indices_timeseries.png"), p_dq_ts, width = 9, height = 10, dpi = 300)

# Also save a tidy summary table for paper/appendix
readr::write_csv(dq_ts, file.path(fig_dir, "dq_indices_timeseries_summary.csv"))

message("Done. Outputs written to: ", fig_dir)
```
