...
###############################################################################
decode_age_years <- function(age_code) {
  age_chr <- as.character(age_code)

  age_unit <- substr(age_chr, 1, 1)           # “1” = years, “2” = months, …
  age_val  <- as.numeric(substr(age_chr, 2, 4))

  dplyr::case_when(
    age_unit == "1" ~  age_val,                       # years
    age_unit == "2" ~  age_val / 12,                  # months → years
    age_unit == "3" ~  age_val / 365,                 # days   → years
    age_unit == "4" ~  age_val / (365 * 24),          # hours  → years
    age_unit == "5" ~  age_val / (365 * 24 * 60),     # minutes→ years
    TRUE              ~ NA_real_                      # “9” = unknown
  )
}
...
summarise_year <- function(yr) {
  ...
  # Read the data
  mortality <- arrow::read_parquet(file, col_select = c("age", "ager52")) %>%
    collect()
  ...
  age_10_99 <- mortality %>%
    filter(age_years >= 10, age_years <= 99)
