...
# Author: Generated by ChatGPT – revised to use temporally stable county IDs
# Date: Sys.Date()
...
# 0.  PACKAGES ------------------------------------------------------------------
# install.packages(c("tidyverse", "solitude", "dbscan", "factoextra", "patchwork", "here"))
...
quality_vars <- c(
  "prop_garbage", "prop_light", "pct_acc_miss", "pct_overd_miss", "prop_all_comp"
)
...
res <- res %>%
  mutate(
    direction_score =
      z_prop_garbage + z_prop_light +
      z_pct_acc_miss + z_pct_overd_miss -
      z_prop_all_comp
  )

```
calculate direction scores and make maps
```{r}
# ──────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────
dir_scores <- res %>%               # 'res' is from your anomaly pipeline
...
map_plot <- make_map(
  ...
  title   = "Direction Score (1999–2022)"
)
