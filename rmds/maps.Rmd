...
vars_to_map <- c(
  "prop_garbage", "DQ_rec_ig_frac_mean_garbage", "DQ_overall", "DQ_rec_ig_abs_mean", "foreman_garbage"
)
...
make_map <- function(sf_data, var, title = NULL, palette = "RdBu") {
  ...
  fill_scale <- if (var == "prop_garbage") {
    ...
    scale_fill_distiller(palette = palette, limits = c(0, 0.03), oob = scales::squish, direction = -1, na.value = "grey90")
...
```
Diagnostics of COD standardized diversity maps
```{r}
# ──────────────────────────────────────────────────────────────
# COD standardized diversity ~ Age composition (county → cluster)
#   • Builds county age shares from MCOD parquet (death-weighted)
#   • Aggregates to temporally-stable clusters by period windows
#   • Joins to cluster_metrics_ucr39_cstd.csv.gz (your CSTD metrics)
#   • Outputs:
#       - figures/cstd_age/scatter_<metric>_vs_age65_4panel.png
#       - figures/cstd_age/scatter_<metric>_vs_age0_14_4panel.png
#       - output/cstd_age/regression_<metric>.csv  (period-FE OLS)
#       - (optional) figures/cstd_age/maps/<metric>_by_age65_quartile_4panel.png
# ──────────────────────────────────────────────────────────────

suppressPackageStartupMessages({
  library(readr);  library(dplyr);  library(stringr);  library(tidyr);  library(tibble)
  library(purrr);   library(ggplot2); library(scales); library(here);   library(arrow)
  library(sf);      library(tigris);  library(broom)
})

options(tigris_use_cache = TRUE, tigris_class = "sf", sf_use_s2 = TRUE)

# ---------------------- configurable paths --------------------
mcod_dir        <- here("data_private", "mcod_sample")            # parquet files: mcod_YYYY.parquet
metrics_file    <- here("output", "cluster_metrics_ucr39_cstd.csv.gz")
membership_file <- here("output", "county_cluster_membership.csv.gz")
out_dir_fig     <- here("figures", "cstd_age")
out_dir_reg     <- here("output", "cstd_age")
dir.create(out_dir_fig, recursive = TRUE, showWarnings = FALSE)
dir.create(out_dir_reg, recursive = TRUE, showWarnings = FALSE)

# ---------------------- period windows ------------------------
# Use same levels/labels as your mapping script
period_levels <- c("1999_2005","2006_2012","2013_2019","2020_2022")

assign_period <- function(y) {
  dplyr::case_when(
    y >= 1999 & y <= 2005 ~ "1999_2005",
    y >= 2006 & y <= 2012 ~ "2006_2012",
    y >= 2013 & y <= 2019 ~ "2013_2019",
    y >= 2020 & y <= 2022 ~ "2020_2022",
    TRUE ~ NA_character_
  )
}
...
age_bucket <- function(age_years) {
  # Edit boundaries if you prefer different bins
  cut(
    pmax(pmin(age_years, 120), 0),  # clamp 0..120
    breaks = c(-Inf, 14, 44, 64, 84, Inf),
    labels = c("age0_14","age15_44","age45_64","age65_84","age85p"),
    right = TRUE
  )
}

# Map helpers reused for optional stratified maps
crs_proj <- 2163
st_shift <- function(sf_obj, shift=c(0,0)) { st_geometry(sf_obj) <- st_geometry(sf_obj)+shift; sf_obj }
st_scale <- function(sf_obj, scale=1) {
  centroid <- st_centroid(st_union(sf_obj))
...
# Which metric columns? (keep it flexible; drop keys)
metric_cols <- names(metrics)[vapply(metrics, is.numeric, logical(1))]
metric_cols <- setdiff(metric_cols, c("fips","GEOID","STATEFP","COUNTYFP"))

# ---------------- build county-year age shares from MCOD -------
# Reads only needed cols; expects files named like mcod_YYYY.parquet in mcod_dir
years <- 1999:2022
mcod_paths <- file.path(mcod_dir, paste0("mcod_", years, ".parquet"))
mcod_paths <- mcod_paths[file.exists(mcod_paths)]
stopifnot(length(mcod_paths) > 0)

message("Computing county-year age shares from MCOD (", length(mcod_paths), " files)")
age_cy <- map_dfr(mcod_paths, function(pth) {
  yr <- as.integer(stringr::str_extract(basename(pth), "\\d{4}"))
  ds <- arrow::read_parquet(pth, as_data_frame = TRUE,
                            col_select = c("age_years","county_ihme","year")) %>%
    rename(fips_raw = county_ihme) %>%
    mutate(year = coalesce(as.integer(year), yr),
           fips  = stringr::str_pad(as.character(fips_raw), 5, pad="0"),
           abuck = age_bucket(as.numeric(age_years))) %>%
    filter(!is.na(fips), !is.na(year), !is.na(abuck))

  ds %>%
    count(fips, year, abuck, name = "deaths") %>%
    group_by(fips, year) %>%
    mutate(total = sum(deaths), share = deaths / total) %>%
    ungroup() %>%
    select(fips, year, abuck, share, total)
})

# Wide shares per county-year
age_cy_wide <- age_cy %>%
  select(fips, year, abuck, share) %>%
  tidyr::pivot_wider(names_from = abuck, values_from = share, values_fill = 0) %>%
  # for convenience, add combined 65+ and child share
  mutate(age65p = coalesce(age65_84,0) + coalesce(age85p,0),
         age0_14 = coalesce(age0_14,0))

# ---------------- county-year → cluster-period -----------------
# Attach period & cluster (via membership for that period)
age_cp <- age_cy %>%
  distinct(fips, year) %>%
  mutate(period = factor(assign_period(year), levels = period_levels)) %>%
  left_join(age_cy_wide, by = c("fips","year")) %>%
  left_join(membership, by = c("fips","period")) %>%
  filter(!is.na(period), !is.na(cluster))

# Aggregate to cluster-period (death-weighted by county-year totals)
age_clper <- age_cy %>%
  left_join(age_cy_wide, by = c("fips","year")) %>%
  mutate(period = factor(assign_period(year), levels = period_levels)) %>%
  left_join(membership, by = c("fips","period")) %>%
  filter(!is.na(period), !is.na(cluster)) %>%
  group_by(cluster, period, fips, year) %>%
  summarise(w = max(total, na.rm = TRUE),
            age0_14 = max(age0_14, na.rm = TRUE),
            age15_44 = max(coalesce(age15_44,0), na.rm = TRUE),
            age45_64 = max(coalesce(age45_64,0), na.rm = TRUE),
            age65_84 = max(coalesce(age65_84,0), na.rm = TRUE),
            age85p   = max(coalesce(age85p,0), na.rm = TRUE),
            age65p   = age65_84 + age85p,
            .groups = "drop") %>%
  group_by(cluster, period) %>%
  summarise(
    wsum = sum(w, na.rm = TRUE),
    age0_14 = weighted.mean(age0_14, w, na.rm = TRUE),
    age15_44 = weighted.mean(age15_44, w, na.rm = TRUE),
    age45_64 = weighted.mean(age45_64, w, na.rm = TRUE),
    age65_84 = weighted.mean(age65_84, w, na.rm = TRUE),
    age85p   = weighted.mean(age85p,   w, na.rm = TRUE),
    age65p   = weighted.mean(age65p,   w, na.rm = TRUE),
    .groups = "drop"
  )

# ---------------- join age with CSTD metrics -------------------
dat <- metrics %>%
  select(cluster, period, all_of(metric_cols)) %>%
  left_join(age_clper, by = c("cluster","period"))

# ---------------- quick diagnostics: scatter + GAM -------------
dir.create(file.path(out_dir_fig, "scatter"), showWarnings = FALSE)
plot_scatter <- function(df, metric, xvar, xlab, fname_suffix) {
  df_plot <- df %>% select(cluster, period, value = all_of(metric), x = all_of(xvar)) %>% filter(is.finite(value), is.finite(x))
  ...
  fout <- file.path(out_dir_fig, "scatter", paste0("scatter_", sanitize(metric), "_vs_", fname_suffix, "_4panel.png"))
  ggsave(fout, p, width = 10, height = 7.2, dpi = 320)
  message("Saved: ", fout)
}

# Choose a small set if you have many metrics; else loop all
metrics_to_check <- metric_cols
for (m in metrics_to_check) {
  plot_scatter(dat, metric = m, xvar = "age65p", xlab = "Share aged 65+", fname_suffix = "age65p")
  plot_scatter(dat, metric = m, xvar = "age0_14", xlab = "Share aged 0–14", fname_suffix = "age0_14")
}

# ---------------- regressions: metric ~ age shares + period FE --
# Saves tidy tables per metric
run_reg <- function(df, metric) {
  dd <- df %>%
    select(period, value = all_of(metric), age0_14, age15_44, age45_64, age65p) %>%
    filter(is.finite(value), across(c(age0_14,age15_44,age45_64,age65p), is.finite)) %>%
    mutate(period = factor(period, levels = period_levels))
  if (!nrow(dd)) return(invisible(NULL))
  # Drop one age share to avoid perfect collinearity; keep age65p + age0_14 + age15_44, omit age45_64
  fit <- lm(value ~ age65p + age0_14 + age15_44 + period, data = dd)
  tib <- broom::tidy(fit, conf.int = TRUE)
  tib$metric <- metric
  fout <- file.path(out_dir_reg, paste0("regression_", sanitize(metric), ".csv"))
  readr::write_csv(tib, fout)
  message("Saved: ", fout)
}
invisible(lapply(metrics_to_check, function(m) run_reg(dat, m)))

# --------------- OPTIONAL: stratified maps by age quartiles ----
make_stratified_maps <- FALSE   # ← set TRUE to produce maps
age_var_for_map      <- "age65p"
metric_for_map       <- if (length(metrics_to_check)) metrics_to_check[1] else NA_character_

if (make_stratified_maps && !is.na(metric_for_map)) {
  counties_tf <- build_counties_2163_resized()
  states_tf   <- build_states_2163_resized()

  # County-period age quantiles, then cluster polygons colored by metric within age strata
  age_q <- age_cp %>%
    group_by(period) %>%
    mutate(age_q = ntile(.data[[age_var_for_map]], 4)) %>%
    ungroup() %>%
    select(fips, period, age_q)

  # Build cluster polygons per period
  build_cluster_sf <- function(period_key, membership_df, counties_sf) {
    mm <- membership_df %>%
      filter(.data$period == period_key) %>%
      transmute(fips = str_pad(as.character(fips), 5, pad = "0"),
                cluster = as.character(cluster)) %>%
      distinct()
    counties_sf %>%
      inner_join(mm, by = "fips") %>%
      group_by(cluster) %>%
      summarise(geometry = sf::st_union(geometry), .groups = "drop") %>%
      sf::st_make_valid()
  }

  # Stack 4 periods; facet by period, color by metric; small multiples for age quartiles
  for (aq in 1:4) {
    stacked_sf <- map_dfr(period_levels, function(win) {
      cl_sf <- build_cluster_sf(win, membership, counties_tf)
      met   <- metrics %>% filter(period == win) %>% select(cluster, !!sym(metric_for_map)) %>% rename(value = !!sym(metric_for_map))
      sf_cl <- cl_sf %>% left_join(met, by = "cluster")
      sf_cl$period <- win
      sf_cl
    }) %>% mutate(period = factor(period, levels = period_levels))

    # Keep clusters whose member counties fall in this age quartile (majority rule)
    cl_age_q <- age_q %>%
      left_join(membership, by = c("fips","period")) %>%
      group_by(cluster, period) %>%
      summarise(age_q_mode = as.integer(names(which.max(table(age_q)))), .groups = "drop")

    stacked_sf <- stacked_sf %>%
      left_join(cl_age_q, by = c("cluster","period")) %>%
      filter(age_q_mode == aq)

    vals <- stacked_sf$value[is.finite(stacked_sf$value)]
    if (!length(vals) || length(unique(vals)) == 1) next
    q <- quantile(vals, c(0.02, 0.98), na.rm = TRUE)
    lims <- c(q[1], q[2])

    p <- ggplot() +
      geom_sf(data = stacked_sf, aes(fill = value), colour = NA) +
      geom_sf(data = states_tf, fill = NA, colour = "white", linewidth = 0.3) +
      scale_fill_distiller(palette = "RdBu", direction = 1, limits = lims, oob = squish, na.value = "grey90") +
      coord_sf(xlim = c(-2500000, 2500000), ylim = c(-2200000, 730000), expand = FALSE) +
      facet_wrap(~ period, ncol = 2) +
      labs(fill = NULL, title = paste0(metric_for_map, " — quartile ", aq, " of ", age_var_for_map)) +
      theme_void() + theme(strip.text = element_text(size = 12, face = "bold"),
                           legend.text = element_text(size = 9))
    fout <- file.path(out_dir_fig, "maps",
                      paste0(sanitize(metric_for_map), "_by_", sanitize(age_var_for_map), "_q", aq, "_4panel.png"))
...
message("Done. See figures/cstd_age/* and output/cstd_age/*.csv")
```
Validation: 4 metrics by public health spending
```{r}
# ──────────────────────────────────────────────────────────────
# 3 line plots: Metric vs PH-spend quintile, 5 lines = Income quintiles
# Metrics: prop_garbage | overdoses unspecified | Phillips detail (UCOD, ICD-4)
# Output: figures/ph_spend_relationship/{prop_garbage,overd_unspec,detail_icd4}_lines.png
# ──────────────────────────────────────────────────────────────
suppressPackageStartupMessages({
  library(dplyr); library(tidyr); library(ggplot2); library(scales); library(here)
})
...
# ---------- prerequisites ----------
stopifnot(exists("cy"), exists("ph_pc"), exists("detail_long"))
stopifnot(!is.na(prop_garbage_col), !is.na(overd_col))
stopifnot("detail_icd4_ucod" %in% names(detail_long))

# Population weighting (optional)
pop_col_ok <- exists("pop_col") && !is.na(pop_col) && pop_col %in% names(cy)
message(if (pop_col_ok) paste0("Population weighting ON (", pop_col, ").")
        else "Population weighting OFF.")

# ---------- income (for 5 colored lines) ----------
get_income_quintiles <- function(cy_df) {
  # 1) try to find an income column in cy
  ...
  # 2) fallback: pull median household income from ACS (B19013_001, ACS5 2022)
  if (requireNamespace("tidycensus", quietly = TRUE)) {
    ...
    inc_any <- inc_acs %>%
      transmute(county_ihme = GEOID, income = as.numeric(estimate)) %>%
      filter(is.finite(income))
    qb_inc <- make_quintile_breaks(inc_any$income)
    return(inc_any %>%
      mutate(income_q = cut(income, breaks = qb_inc, include.lowest = TRUE,
  ...
  stop("No income available in `cy`, and {tidycensus} not installed. ",
       "Either add an income column to `cy` or install {tidycensus} and set a Census API key.")
}

income_quint <- get_income_quintiles(cy)

# ---------- per-county metric averages (1999–2022) ----------
base_sel <- cy %>%
  mutate(
    pg  = suppressWarnings(as.numeric(.data[[prop_garbage_col]])),
    od  = suppressWarnings(as.numeric(.data[[overd_col]])),
    pop = if (pop_col_ok) suppressWarnings(as.numeric(.data[[pop_col]])) else NA_real_
  ) %>%
  filter(year >= 1999, year <= 2022)
...
county_avg <- base_sel %>%
  group_by(county_ihme) %>%
  summarise(
    prop_garbage = weighted_mean_na(pg,  pop),
    overd_unspec = weighted_mean_na(od,  pop),
    pop_wt       = if (pop_col_ok) mean(pop, na.rm = TRUE) else NA_real_,
    .groups = "drop"
  )

detail_avg <- detail_long %>%
  mutate(detail_icd4_ucod = suppressWarnings(as.numeric(detail_icd4_ucod))) %>%
  group_by(county_ihme) %>%
  summarise(detail_icd4 = mean(detail_icd4_ucod, na.rm = TRUE), .groups = "drop")

# ---------- per-county PH spend (median across finance years) → x-axis quintiles ----------
ph_any <- ph_pc %>%
  group_by(county_ihme) %>%
...
# ---------- combine + summarise: mean metric by spend_q × income_q ----------
comb <- county_avg %>%
  left_join(detail_avg,  by = "county_ihme") %>%
  inner_join(spend_quint, by = "county_ihme") %>%
  inner_join(income_quint, by = "county_ihme")

wmaybe <- function(x, w) if (pop_col_ok) weighted_mean_na(x, w) else mean(x, na.rm = TRUE)

by_q <- comb %>%
  pivot_longer(cols = c(prop_garbage, overd_unspec, detail_icd4),
  ...
  mutate(
    metric = recode(metric,
      ...
      overd_unspec = "Overdoses unspecified (share)",
      detail_icd4  = "Phillips detail (UCOD, ICD-4)"),
    spend_q  = factor(spend_q,  levels = labs_quint),
    income_q = factor(income_q, levels = labs_quint)
  )

# ---------- line-plot helper (x = PH-spend quintile; 5 lines = income quintiles) ----------
plot_lines <- function(df, mname, ylab, file_out) {
  dd <- df %>% filter(metric == mname)
  if (!nrow(dd)) stop("No data for metric: ", mname)
...
p_overd <- plot_lines(by_q, "Overdoses unspecified (share)",   "Overdoses unspecified (share)",  "overd_unspec_lines.png")
p_det   <- plot_lines(by_q, "Phillips detail (UCOD, ICD-4)",   "Phillips detail (UCOD, ICD-4)",  "detail_icd4_lines.png")

# Optional combined 3-panel
suppressPackageStartupMessages(library(patchwork))
combined <- (p_garb / p_overd / p_det) + plot_layout(heights = c(1,1,1))
ggsave(here("figures","ph_spend_relationship","metrics_vs_phspend_lines_3panel.png"),
       combined, width = 9.5, height = 12, dpi = 320)

# Show in Viewer
p_garb; p_overd; p_det

```
Validation: 4 metrics by reporting type
```{r}
# ──────────────────────────────────────────────────────────────
# Time series by REPORTING TYPE (3 plots)
#   • Metrics: prop_garbage | overdoses unspecified | Phillips detail (UCOD, ICD-4)
#   • X = year (1999–2022), lines = reporting type
#   • Population-weighted means if population is available in `cy`
# Saves: figures/reporting_type_timeseries/*.png + combined 3-panel
# ──────────────────────────────────────────────────────────────
suppressPackageStartupMessages({
...
get_reporting_lookup <- function() {
  ...
  if (is.na(path)) {
    stop("No reporting-type lookup found. Provide `rep_lu` (county_ihme, reporting_type) ",
         "or place the CSV under data_raw/.")
  }
...
# normalize reporting types and include "Other County Official"
normalize_reporting_type <- function(x) {
  ...
  dplyr::case_when(
    ...
    grepl("other", y)                              ~ "Other County Official",   # catch "other county official(s)" etc.
...
rep_lookup <- rep_lookup %>%
  dplyr::mutate(
    reporting_type = normalize_reporting_type(reporting_type),
    reporting_type = factor(
      reporting_type,
      levels = c("Coroner","Other County Official","Mixed","Medical Examiner")
    )
  ) %>%
  dplyr::filter(!is.na(reporting_type))

# and keep levels in this order for plots
rt_levels <- levels(rep_lookup$reporting_type)


# ---------- assemble county-year series ----------
base_sel <- cy %>%
  mutate(
    prop_garbage = suppressWarnings(as.numeric(.data[[prop_garbage_col]])),
    overd_unspec = suppressWarnings(as.numeric(.data[[overd_col]])),
    population   = if (pop_col_ok) suppressWarnings(as.numeric(.data[[pop_col]])) else NA_real_
  ) %>%
  filter(year >= 1999, year <= 2022) %>%
  select(county_ihme, year, prop_garbage, overd_unspec, population)

detail_year <- detail_long %>%
  transmute(county_ihme, year,
            detail_icd4 = suppressWarnings(as.numeric(detail_icd4_ucod)))
...
# ---------- aggregate to year × reporting_type ----------
wmaybe <- function(x, w) if (pop_col_ok) weighted_mean_na(x, w) else mean(x, na.rm = TRUE)
...
ts_rt <- series_rt %>%
  ...
  summarise(
    ...
    overd_unspec = wmaybe(overd_unspec, population),
...
plot_metric_rt <- function(df, var, title, file_out) {
  ...
  if (!nrow(dd)) stop("No finite data for ", var)
...
p2 <- plot_metric_rt(ts_rt, "overd_unspec", "Overdoses unspecified (share)",  "overd_unspec_by_reporting_timeseries.png")
p3 <- plot_metric_rt(ts_rt, "detail_icd4",  "Phillips detail (UCOD, ICD-4)",  "detail_icd4_by_reporting_timeseries.png")

# ---------- combined 3-panel (optional) ----------
suppressPackageStartupMessages(library(patchwork))
combined <- (p1 / p2 / p3) + plot_layout(heights = c(1,1,1))
ggsave(here("figures","reporting_type_timeseries","metrics_by_reporting_timeseries_3panel.png"),
       combined, width = 9.5, height = 12, dpi = 320)
