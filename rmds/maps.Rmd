...
```
Map DQ scores with light clustering
```{r}
# ──────────────────────────────────────────────────────────────
...
# per-capita public health spend (county, finance years)
ph_pc_candidates <- c(
...
# optional income-all file (avg county income by ACS year)
income_candidates <- c(
...
if (file.exists(here::here("data_raw","ihme_fips.rda"))) {
  ...
  if (exists("ihme_fips") && all(c("orig_fips","ihme_fips") %in% names(ihme_fips))) {
    ihme_map <- ihme_fips %>%
      transmute(
        fips        = std_fips(orig_fips),
        county_ihme = std_fips(ihme_fips)
      ) %>%
      distinct()
  }
}
...
# ---------- merge → compute z-scores (flip signs for OD & garbage) ----------
metrics_clu <- cluster_core %>%
...
# =================================================================
# B) By PH spend quintile (per-capita public-health expenditure)
#     Source priority:
#       1) ph_pc_file (wide or long) with columns {county_ihme or fips, ph_pc [, fin_year]}
#       2) columns inside cy named like PH spend per-capita
# =================================================================
get_ph_any <- function() {
  if (!is.na(ph_pc_file)) {
    ph_raw <- readr::read_csv(ph_pc_file, show_col_types = FALSE)
    stopifnot(any(c("county_ihme","fips") %in% names(ph_raw)))
    stopifnot("ph_pc" %in% names(ph_raw))
    ph_df <- ph_raw %>%
      mutate(county_ihme = if ("county_ihme" %in% names(.)) std_fips(county_ihme) else std_fips(fips),
             ph_pc = suppressWarnings(as.numeric(ph_pc))) %>%
      group_by(county_ihme) %>%
      summarise(ph_pc = median(ph_pc, na.rm = TRUE), .groups = "drop") %>%
      filter(is.finite(ph_pc))
    if (nrow(ph_df)) return(ph_df)
  }
  # fallback: try to detect PH-spend-per-capita in cy
  cand <- c("ph_pc","ph_per_capita","phspend_pc","ph_spend_pc","public_health_spend_pc","ph_pc_usd")
  ccol <- cand[cand %in% names(cy)][1]
  if (!is.na(ccol)) {
    message("Using PH spend from cy$", ccol, " (county median across years).")
    return(
      cy %>%
        transmute(county_ihme, ph_pc = suppressWarnings(as.numeric(.data[[ccol]]))) %>%
        group_by(county_ihme) %>%
        summarise(ph_pc = median(ph_pc, na.rm = TRUE), .groups = "drop") %>%
        filter(is.finite(ph_pc))
    )
  }
  stop("No PH spend data found. Provide ph_pc.csv or add a PH-per-capita column to cy.")
}
ph_any <- get_ph_any()
...
ts_spend <- dir_long %>%
  ...
  group_by(period, spend_q) %>%
  summarise(mean_direction = mean(direction_score, na.rm = TRUE),
            n = dplyr::n(), .groups = "drop") %>%
  mutate(spend_q = factor(spend_q, levels = labs_quint))

p_spend <- ggplot(ts_spend, aes(x = period, y = mean_direction, group = spend_q, colour = spend_q)) +
  geom_line(linewidth = 1.2) + geom_point(size = 2) +
  labs(title = "Z-score sum by PH spend (per-capita) quintile",
       x = NULL, y = "Mean direction_score", colour = "PH spend quintile") +
  theme_bw(base_size = 12)
ggsave(here("figures","validation_zscore","direction_by_spend_quintile.png"),
       p_spend, width = 9.5, height = 5.2, dpi = 320)

# =================================================================
# C) By INCOME quintile (average county income)
#     Source priority:
#       1) income_all.csv with columns {county_ihme, avg_income}
#       2) an income column inside cy (one of several common names)
# =================================================================
get_income_quintiles <- function() {
  if (!is.na(income_file)) {
    inc_df <- readr::read_csv(income_file, show_col_types = FALSE)
    stopifnot(all(c("county_ihme","avg_income") %in% names(inc_df)))
    out <- inc_df %>%
      transmute(county_ihme = std_fips(county_ihme),
                income = suppressWarnings(as.numeric(avg_income))) %>%
      group_by(county_ihme) %>% summarise(income = mean(income, na.rm = TRUE), .groups = "drop")
    return(out %>% filter(is.finite(income)))
  }
  # fallback to cy columns if present
  nms <- names(cy)
  cand <- c("median_household_income","median_income","mhi","hh_income",
            "income_pc","per_capita_income","pc_income","income")
  inc_col <- cand[cand %in% nms][1]
  if (is.na(inc_col)) stop("No income file found and no income column in `cy`.")
  cy %>%
    transmute(county_ihme, income = suppressWarnings(as.numeric(.data[[inc_col]]))) %>%
    group_by(county_ihme) %>% summarise(income = median(income, na.rm = TRUE), .groups = "drop") %>%
    filter(is.finite(income))
}
inc_any <- get_income_quintiles()
qb_inc  <- make_quintile_breaks(inc_any$income)
income_quint <- inc_any %>%
  mutate(income_q = cut(income, breaks = qb_inc, include.lowest = TRUE, right = FALSE, labels = labs_quint)) %>%
  select(county_ihme, income_q)

ts_income <- dir_long %>%
  inner_join(income_quint, by = "county_ihme") %>%
  group_by(period, income_q) %>%
  summarise(mean_direction = mean(direction_score, na.rm = TRUE),
            n = dplyr::n(), .groups = "drop") %>%
  mutate(income_q = factor(income_q, levels = labs_quint))

p_income <- ggplot(ts_income, aes(x = period, y = mean_direction, group = income_q, colour = income_q)) +
  geom_line(linewidth = 1.2) + geom_point(size = 2) +
  labs(title = "Z-score sum by income quintile",
       x = NULL, y = "Mean direction_score", colour = "Income quintile") +
  theme_bw(base_size = 12)
ggsave(here("figures","validation_zscore","direction_by_income_quintile.png"),
       p_income, width = 9.5, height = 5.2, dpi = 320)

# ---------- Combined 3-panel ----------
suppressPackageStartupMessages(library(patchwork))
combined <- (p_rep / p_spend / p_income) + plot_layout(heights = c(1,1,1))
ggsave(here("figures","validation_zscore","direction_timeseries_3panel.png"),
       combined, width = 10, height = 12, dpi = 320)

# Show in Viewer (optional)
p_rep; p_spend; p_income
...
# 1.  LOAD & CLEAN DATA --------------------------------------------------------
df <- read_csv(
  here("data", "county_year_quality_metrics.csv"),
  show_col_types = FALSE
) %>%
  mutate(
    year = as.integer(year)
