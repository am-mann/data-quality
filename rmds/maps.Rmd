...
```{r}
# Simplified validation summary: R^2 and means by ratioMeanUCD quintile
suppressPackageStartupMessages({
  library(dplyr); library(tidyr); library(readr); library(stringr); library(here)
})

dir.create(here("figures","covid_validation"), recursive = TRUE, showWarnings = FALSE)

std_fips <- function(x) stringr::str_pad(as.character(x), 5, pad = "0")
`%||%` <- function(a,b) if (!is.null(a) && length(a) && !is.na(a)) a else b
safe_wmean <- function(x, w) {
  xx <- ifelse(is.finite(x), x, NA_real_); ww <- ifelse(is.finite(w), w, NA_real_)
  if (all(is.na(xx))) return(NA_real_)
  if (all(is.na(ww)) || sum(ww, na.rm = TRUE) <= 0) return(mean(xx, na.rm = TRUE))
  sum(xx * ww, na.rm = TRUE) / sum(ww, na.rm = TRUE)
}
...
if (is.na(prop_garbage_col)) stop("prop_garbage column not found in `cy`.")

# determine weight column if available
w_col <- if (exists("pop_col") && !is.na(pop_col) && pop_col %in% names(cy)) as.character(pop_col) else NA_character_
if (is.na(w_col) && "population" %in% names(cy)) w_col <- "population"
if (is.na(w_col) && "n_cert"     %in% names(cy)) w_col <- "n_cert"
w_label <- ifelse(!is.na(w_col), w_col, "UNWEIGHTED")

# --- read estimatesMonthly.csv and compute mean ratio per county (Mar2020 - Aug2022) ---
est_file <- here("data_raw", "estimatesMonthly.csv")
if (!file.exists(est_file)) stop("Cannot find ", est_file)
est_raw <- readr::read_csv(est_file, show_col_types = FALSE)

ratio_col <- "ratioMedUCD"

est_clean <- est_raw %>%
  transmute(
    county_ihme = std_fips(.data[["FIPSCode"]]),
    year = suppressWarnings(as.integer(.data[["year"]])),
    month = suppressWarnings(as.integer(.data[["month"]])),
    ratioMeanUCD = suppressWarnings(as.numeric(.data[[ratio_col]]))
  ) %>%
  filter(!is.na(county_ihme), is.finite(year), is.finite(month), is.finite(ratioMeanUCD)) %>%
  mutate(ym = year * 100L + month) %>%
  filter(ym >= 202003L, ym <= 202208L)

ratio_by_county <- est_clean %>%
  group_by(county_ihme) %>%
  summarise(ratioMeanUCD_avg = mean(ratioMeanUCD, na.rm = TRUE), .groups = "drop") %>%
  filter(is.finite(ratioMeanUCD_avg))

# quintiles of ratio
qb_ratio <- make_quintile_breaks(ratio_by_county$ratioMeanUCD_avg)
ratio_quint <- ratio_by_county %>%
  mutate(ratio_q = cut(ratioMeanUCD_avg, breaks = qb_ratio, include.lowest = TRUE, right = FALSE, labels = labs_quint))

# --- build county-level stable averages (weighted across years) ---
# create detail_year if not present (same logic as your original script)
if (!exists("detail_year")) {
  detail_long <- detail_long %>% mutate(county_ihme = std_fips(county_ihme))
  ucod_icd4_candidates <- c("detail_icd4_ucod","detail_ucod_icd4_cstd","detail_ucod_icd4","detail_icd4")
  wide_hit <- intersect(ucod_icd4_candidates, names(detail_long))[1]
  if (!is.na(wide_hit)) {
    has_year <- "year" %in% names(detail_long)
    detail_year <- detail_long %>% transmute(
      county_ihme, year = if (has_year) suppressWarnings(as.integer(year)) else NA_integer_,
      detail_icd4 = suppressWarnings(as.numeric(.data[[wide_hit]]))
    )
  } else {
    stopifnot(all(c("metric","value") %in% names(detail_long)))
    ...
    dl <- detail_long %>% dplyr::filter(grepl(icd4_pat, tolower(metric)))
    detail_year <- dl %>% transmute(
      county_ihme, year = if ("year" %in% names(dl)) suppressWarnings(as.integer(year)) else NA_integer_,
      detail_icd4 = suppressWarnings(as.numeric(value))
    ) %>% group_by(county_ihme, year) %>% summarise(detail_icd4 = mean(detail_icd4, na.rm = TRUE), .groups = "drop")
  }
  detail_year <- detail_year %>% filter(is.finite(year), is.finite(detail_icd4)) %>% mutate(year = as.integer(year))
}

# weights per county-year (for aggregation)
w_df <- cy %>%
  transmute(county_ihme = std_fips(county_ihme), year,
            weight = if (!is.na(w_col) && w_col %in% names(cy)) suppressWarnings(as.numeric(.data[[w_col]])) else NA_real_) %>%
  filter(is.finite(year), year >= 1999, year <= 2022)

# base selection
base_sel <- cy %>%
  transmute(county_ihme = std_fips(county_ihme), year,
            prop_garbage   = suppressWarnings(as.numeric(.data[[prop_garbage_col]])),
            ri             = if ("RI_post_only" %in% names(cy)) suppressWarnings(as.numeric(.data[["RI_post_only"]])) else NA_real_) %>%
  left_join(w_df, by = c("county_ihme","year")) %>%
  filter(year >= 1999, year <= 2022)
...
# join ratio info
county_avg <- county_avg %>% inner_join(ratio_quint, by = "county_ihme")

# restrict to counties with finite metric & ratio
metrics <- c("prop_garbage","detail_icd4","ri")
valid_counties <- county_avg %>% filter(if_all(all_of(metrics), ~ TRUE)) # keep all; filter per-metric later

# helper: fit (weighted if available) lm and return R^2 (summary()$r.squared)
fit_r2 <- function(df, response_col, weight_col = "wt_sum") {
  df2 <- df %>% filter(is.finite(.data[[response_col]]), is.finite(ratioMeanUCD_avg))
  if (nrow(df2) < 3) return(NA_real_)
  wvec <- if (weight_col %in% names(df2) & any(is.finite(df2[[weight_col]]) & df2[[weight_col]] > 0)) df2[[weight_col]] else NULL
  fm <- if (is.null(wvec)) {
    lm(as.formula(paste(response_col, "~ ratioMeanUCD_avg")), data = df2)
  } else {
    lm(as.formula(paste(response_col, "~ ratioMeanUCD_avg")), data = df2, weights = wvec)
  }
  s <- summary(fm)
  # summary(lm) returns R^2 in $r.squared (even for weighted lm this is conventional)
  s$r.squared
}

# Build summary table: for each metric, for Overall and each ratio quintile
out_rows <- list()
for (metric in metrics) {
  # overall
  r2_overall <- fit_r2(county_avg, metric)
  mean_metric_overall <- safe_wmean(county_avg[[metric]], county_avg$wt_sum)
  mean_prop_garbage_overall <- safe_wmean(county_avg$prop_garbage, county_avg$wt_sum)
  n_overall <- sum(is.finite(county_avg[[metric]]) & is.finite(county_avg$ratioMeanUCD_avg))
  out_rows[[length(out_rows)+1]] <- tibble::tibble(
    metric = metric,
    quintile = "Overall",
    n_counties = n_overall,
    mean_metric = mean_metric_overall,
    mean_prop_garbage = mean_prop_garbage_overall,
    R2 = r2_overall
  )

  # by quintile
  for (q in labs_quint) {
    sub <- county_avg %>% filter(ratio_q == q)
    n_sub <- nrow(sub %>% filter(is.finite(.data[[metric]]), is.finite(ratioMeanUCD_avg)))
    if (n_sub < 3) {
      r2_q <- NA_real_
    } else {
      r2_q <- fit_r2(sub, metric)
    }
    out_rows[[length(out_rows)+1]] <- tibble::tibble(
      metric = metric,
      quintile = q,
      n_counties = n_sub,
      mean_metric = safe_wmean(sub[[metric]], sub$wt_sum),
      mean_prop_garbage = safe_wmean(sub$prop_garbage, sub$wt_sum),
      R2 = r2_q
    )
  }
}
...
summary_tbl <- dplyr::bind_rows(out_rows) %>%
  mutate(
    metric = dplyr::recode(metric,
      prop_garbage = "Proportion garbage (UCOD)",
      detail_icd4 = "Phillips detail (UCOD, ICD-4)",
      ri = "Re-assignability Index (RI)"
    )
  ) %>%
  arrange(metric, factor(quintile, levels = c("Overall", labs_quint)))

# print nicely to console
print(summary_tbl)

# save CSV
readr::write_csv(summary_tbl, here("figures","covid_validation","validation_summary_by_ratio_quintile.csv"))

# also return the summary object invisibly if used in a script
invisible(summary_tbl)

```
```{r}
# Compute R2 + weighted correlation, p-value, CI for 3 metrics + aggregate
suppressPackageStartupMessages({
  library(dplyr); library(readr); library(stringr); library(here)
})
...
safe_wvar <- function(x, w) {
  xx <- ifelse(is.finite(x), x, NA_real_); ww <- ifelse(is.finite(w), w, NA_real_)
  ok <- !is.na(xx) & !is.na(ww) & ww > 0
  if (sum(ok) <= 1) return(NA_real_)
  wm <- sum(ww[ok] * xx[ok]) / sum(ww[ok])
  sum(ww[ok] * (xx[ok] - wm)^2) / sum(ww[ok])
}
safe_wsd <- function(x, w) sqrt(safe_wvar(x,w))

# weighted Pearson correlation + Fisher CI/p-value
w_pearson <- function(x, y, w = NULL) {
  ok <- is.finite(x) & is.finite(y)
  if (!is.null(w)) ok <- ok & is.finite(w) & (w > 0)
  x <- x[ok]; y <- y[ok]; if (!is.null(w)) w <- w[ok]
  n <- length(x); if (n < 4) return(c(r=NA,p=NA,lo=NA,hi=NA))
  if (is.null(w) || length(w)==0) {
    r <- cor(x, y)
  } else {
    wm <- function(z, w) sum(w*z)/sum(w)
    xm <- wm(x,w); ym <- wm(y,w)
    covw <- sum(w*(x-xm)*(y-ym))/sum(w)
    sx <- sqrt(sum(w*(x-xm)^2)/sum(w)); sy <- sqrt(sum(w*(y-ym)^2)/sum(w))
    r <- covw/(sx*sy)
  }
  # Fisher z-transform for CI and p
  zr <- 0.5*log((1+r)/(1-r))
  se <- 1/sqrt(n-3)
  zcrit <- qnorm(0.975)
  lo <- tanh(zr - zcrit*se); hi <- tanh(zr + zcrit*se)
  tval <- r*sqrt((n-2)/(1-r^2))
  pval <- 2*pt(-abs(tval), df=n-2)
  c(r=r, p=pval, lo=lo, hi=hi)
}
...
cy <- cy %>% mutate(county_ihme = std_fips(county_ihme))
find_col <- function(df, patterns) {
  nms <- names(df); for (pat in patterns) { m <- nms[grepl(pat, tolower(nms))]; if (length(m)) return(m[1]) }
  NA_character_
}
prop_garbage_col <- get0("prop_garbage_col") %||%
  find_col(cy, c("\\bprop[_]?garbage\\b","garbage[_]?share","frac[_]?garbage",
                 "dq_rec_ig_frac_mean_garbage","foreman[_]?garbage","\\bgarbage\\b"))
if (is.na(prop_garbage_col)) stop("prop_garbage column not found in `cy`.")
w_col <- if (exists("pop_col") && !is.na(pop_col) && pop_col %in% names(cy)) as.character(pop_col) else NA_character_
if (is.na(w_col) && "population" %in% names(cy)) w_col <- "population"
...
# ---------- read ratioMeanUCD ----------
est <- read_csv(here("data_raw","estimatesMonthly.csv"), show_col_types=FALSE)
ratio_col <- if ("ratioMeanUCD" %in% names(est)) "ratioMeanUCD" else
  find_col(est, c("ratiomean","ratio_ucd","ratio_mean_ucd"))
est_clean <- est %>%
  transmute(county_ihme=std_fips(FIPSCode),
            year=as.integer(year), month=as.integer(month),
            ratioMeanUCD=as.numeric(.data[[ratio_col]])) %>%
  mutate(ym=year*100L+month) %>%
  filter(ym>=202003L, ym<=202208L, is.finite(ratioMeanUCD))
ratio_by_county <- est_clean %>%
  group_by(county_ihme) %>%
  summarise(ratioMeanUCD_avg=mean(ratioMeanUCD,na.rm=TRUE),.groups="drop")
...
if (!exists("detail_year")) {
  detail_long <- detail_long %>% mutate(county_ihme = std_fips(county_ihme))
  hit <- intersect(c("detail_icd4_ucod","detail_ucod_icd4_cstd","detail_ucod_icd4","detail_icd4"), names(detail_long))[1]
  has_year <- "year" %in% names(detail_long)
  detail_year <- detail_long %>% transmute(
    county_ihme,
    year = if (has_year) as.integer(year) else NA_integer_,
    detail_icd4 = as.numeric(.data[[hit]])
  ) %>% filter(is.finite(year), is.finite(detail_icd4))
}

# ---------- build county averages ----------
w_df <- cy %>% transmute(county_ihme, year,
                         weight=if (!is.na(w_col) && w_col %in% names(cy))
                           as.numeric(.data[[w_col]]) else NA_real_)
base_sel <- cy %>%
  transmute(county_ihme, year,
            prop_garbage=as.numeric(.data[[prop_garbage_col]]),
            ri=if ("RI_post_only" %in% names(cy))
              as.numeric(.data[["RI_post_only"]]) else NA_real_) %>%
  left_join(w_df, by=c("county_ihme","year"))

county_avg <- base_sel %>%
  left_join(detail_year, by=c("county_ihme","year")) %>%
  group_by(county_ihme) %>%
  summarise(prop_garbage=safe_wmean(prop_garbage,weight),
            detail_icd4=safe_wmean(detail_icd4,weight),
            ri=safe_wmean(ri,weight),
            wt_sum=sum(weight,na.rm=TRUE),
            .groups="drop") %>%
  inner_join(ratio_by_county, by="county_ihme")

# ---------- compute R² + correlations ----------
fit_r2 <- function(df, y) {
  df2 <- df %>% filter(is.finite(.data[[y]]), is.finite(ratioMeanUCD_avg))
  if (nrow(df2)<3) return(NA_real_)
  if (any(is.finite(df2$wt_sum)&df2$wt_sum>0))
    summary(lm(reformulate("ratioMeanUCD_avg",y),df2,weights=df2$wt_sum))$r.squared
  else summary(lm(reformulate("ratioMeanUCD_avg",y),df2))$r.squared
}

metrics <- c("prop_garbage","detail_icd4","ri")

get_stats <- function(df, m) {
  df2 <- df %>% filter(is.finite(.data[[m]]), is.finite(ratioMeanUCD_avg))
  n <- nrow(df2)
  if (n<4) return(data.frame(metric=m,n_counties=n,R2=NA,correlation=NA,
                             corr_sq=NA,p_value=NA,ci_lower=NA,ci_upper=NA))
  w_ok <- any(is.finite(df2$wt_sum)&df2$wt_sum>0)
  corrres <- if (w_ok) w_pearson(df2[[m]],df2$ratioMeanUCD_avg,df2$wt_sum)
             else w_pearson(df2[[m]],df2$ratioMeanUCD_avg,NULL)
  r2 <- fit_r2(df2,m)
  data.frame(metric=m,n_counties=n,R2=r2,
             correlation=corrres["r"],corr_sq=corrres["r"]^2,
             p_value=corrres["p"],ci_lower=corrres["lo"],ci_upper=corrres["hi"])
}

results <- lapply(metrics,get_stats,df=county_avg) %>% bind_rows()

# ---------- aggregate metric ----------
for (m in metrics){
  wm<-safe_wmean(county_avg[[m]],county_avg$wt_sum)
  ws<-safe_wsd(county_avg[[m]],county_avg$wt_sum)
  if(!is.finite(ws)||ws<=0) ws<-sd(county_avg[[m]],na.rm=TRUE)
  county_avg[[paste0(m,"_z")]]<-(county_avg[[m]]-wm)/ws
}
county_avg$aggregate_z<-apply(county_avg[,paste0(metrics,"_z")],1,
                              function(r) if(all(is.na(r))) NA else mean(r,na.rm=TRUE))

results <- bind_rows(results,get_stats(county_avg,"aggregate_z"))

# pretty labels
results$metric <- recode(results$metric,
  prop_garbage="Proportion garbage (UCOD)",
  detail_icd4="Phillips detail (UCOD, ICD-4)",
  ri="Re-assignability Index (RI)",
  aggregate_z="Aggregate (mean weighted z)")

# ---------- output ----------
print(results)
write_csv(results, here("figures","covid_validation","r2_summary_continuous_with_corr.csv"))
invisible(results)
