...
```{r}
# ──────────────────────────────────────────────────────────────
# Map ALL cluster-level metrics (using here::here)
# ──────────────────────────────────────────────────────────────
...
make_map <- function(sf_data, fill_col, lims, diverging, title) {
  ...
  p <- ggplot() +
    ...
    geom_sf(data = states, fill = NA, colour = "white", linewidth = 0.3) +
    coord_sf(expand = FALSE) +  # no hard-coded x/ylim → no cropping
...
make_geoid_if_needed <- function(cnty) {
  nm <- names(cnty)
  if (!("GEOID" %in% nm)) {
    if (all(c("STATEFP", "COUNTYFP") %in% nm)) {
      cnty <- cnty %>%
        mutate(GEOID = str_pad(STATEFP, 2, "0") %>% paste0(str_pad(COUNTYFP, 3, "0")))
    } else {
      stop("Could not find GEOID or (STATEFP, COUNTYFP) in county shapefile.")
    }
  }
  cnty
}

# Build cluster polygons for a period (using 2020 counties for all)
build_cluster_sf <- function(period_key, membership_df) {
  mm <- membership_df %>%
  ...
  cnty <- tigris::counties(cb = TRUE, year = 2020, class = "sf") %>%
    make_geoid_if_needed() %>%
...
membership <- read_csv(membership_file, show_col_types = FALSE) %>%
...
for (mcol in metric_cols) {
  ...
  for (win in periods) {
    ...
    cl_sf <- build_cluster_sf(win, membership)
