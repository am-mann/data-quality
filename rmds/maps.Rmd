...
stopifnot(all(c("RI","RI_post_only","n_cert") %in% names(dq)))
...
min_gb <- 2000L   # cluster size threshold
...
join_shapes <- function(metric_df) {
  lapply(names(shapefile_years), function(window) {
    ...
    left_join(shape_all, dplyr::filter(metric_df, time_window == window), by = "cluster_id")
...
# 8) Map helpers with different limits
make_map_RI <- function(sf_data, title) {
  states <- tigris::states(cb = TRUE, class = "sf") |>
    sf::st_transform(crs_proj) |>
    sf::st_make_valid()
  ggplot() +
    geom_sf(data = sf_data, aes(fill = RI), colour = NA) +
    geom_sf(data = states, fill = NA, colour = "white", linewidth = 0.3) +
    scale_fill_distiller(palette = "RdBu", limits = c(0.15, 0.3),
...
make_map_post <- function(sf_data, title) {
  states <- tigris::states(cb = TRUE, class = "sf") |>
    sf::st_transform(crs_proj) |>
    sf::st_make_valid()
  ggplot() +
    geom_sf(data = sf_data, aes(fill = RI_post_only), colour = NA) +
    geom_sf(data = states, fill = NA, colour = "white", linewidth = 0.3) +
    scale_fill_distiller(palette = "RdBu", limits = c(0.05, 0.15),
                         oob = scales::squish, direction = 1,
                         na.value = "grey90") +
    coord_sf(xlim = c(-2500000, 2500000), ylim = c(-2200000, 730000), expand = FALSE) +
    labs(title = title, fill = NULL) +
...
data_candidates <- c(here("data","county_year_quality_metrics.csv.gz"),
