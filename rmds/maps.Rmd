...
make_map <- function(sf_data, var = "RI", title = NULL, palette = "RdBu") {
  ...
  fill_scale <- scale_fill_distiller(
    palette = palette,
    limits = if (var == "RI") c(0.12, 0.3) else NULL,
    oob = scales::squish,
...
```{r}
# ──────────────────────────────────────────────────────────────
# CORRELATIONS: 4 z-score metrics (overall + by reporting / PH spend / income)
#   • Metrics: z_prop_overd_unspec, z_philips_cstd, z_RI_cluster, z_prop_garbage
#   • Weighted by deaths (sum_n_cert) at cluster×period; also saves unweighted
#   • Stratifiers (optional; run if lookups exist):
#       - Reporting type   (modal, death-weighted, county → cluster×period)
#       - PH spend quintile (modal, death-weighted)
#       - Income quintile   (modal, death-weighted)
#   • Outputs:
#       output/validation_zscore/corr_overall.csv
#       output/validation_zscore/corr_by_reporting.csv     (if rep_lookup available)
#       output/validation_zscore/corr_by_ph_spend.csv      (if ph_q available)
#       output/validation_zscore/corr_by_income.csv        (if inc_q available)
# ──────────────────────────────────────────────────────────────
suppressPackageStartupMessages({
  library(dplyr); library(tidyr); library(readr); library(tibble); library(here)
})
dir.create(here("output","validation_zscore"), recursive = TRUE, showWarnings = FALSE)
out_dir <- here("output","validation_zscore")
...
z_cluster <- z_tbl %>%
  ...
  inner_join(cluster_core %>% select(period, cluster, sum_n_cert), by = c("period","cluster")) %>%
...
weighted_cor_mat <- function(df, vars, wcol = NULL, method = c("pearson","spearman")) {
  ...
  # listwise complete rows across vars; handle weights safely
  if (is.null(w)) {
    keep <- stats::complete.cases(X)
  } else {
    keep <- stats::complete.cases(X) & is.finite(w) & w > 0
  }
  ...
  s <- sqrt(diag(C))
  s[!is.finite(s) | s == 0] <- NA_real_  # guard against zero variance
  R <- C / (s %o% s)
...
readr::write_csv(corr_overall, file.path(out_dir, "corr_overall.csv"))
message("[CORR] Saved overall correlations: ", file.path(out_dir, "corr_overall.csv"))
...
if (exists("rep_lookup") && is.data.frame(rep_lookup)) {
  ...
  if (!is.null(cl_rep) && nrow(cl_rep)) {
    ...
    readr::write_csv(corr_by_reporting, file.path(out_dir, "corr_by_reporting.csv"))
    message("[CORR] Saved by-reporting correlations: ", file.path(out_dir, "corr_by_reporting.csv"))
  } else {
    message("[CORR] Reporting-type lookup present but no modal groups could be formed.")
  }
} else {
  message("[CORR] Reporting-type lookup not found; skipping by-reporting correlations.")
}
...
if (exists("ph_q") && is.data.frame(ph_q) && nrow(ph_q)) {
  ...
  if (!is.null(cl_ph) && nrow(cl_ph)) {
    ...
    readr::write_csv(corr_by_ph, file.path(out_dir, "corr_by_ph_spend.csv"))
    message("[CORR] Saved by-PH-spend correlations: ", file.path(out_dir, "corr_by_ph_spend.csv"))
  } else {
    message("[CORR] PH spend quintiles present but no modal groups could be formed.")
  }
} else {
  message("[CORR] PH spend quintiles not available; skipping.")
}
...
if (exists("inc_q") && is.data.frame(inc_q) && nrow(inc_q)) {
  ...
  if (!is.null(cl_inc) && nrow(cl_inc)) {
    ...
    readr::write_csv(corr_by_inc, file.path(out_dir, "corr_by_income.csv"))
    message("[CORR] Saved by-income correlations: ", file.path(out_dir, "corr_by_income.csv"))
  } else {
    message("[CORR] Income quintiles present but no modal groups could be formed.")
  }
} else {
  message("[CORR] Income quintiles not available; skipping.")
}
...
ri_col    <- find_col(cy, c("^ri$","\\bri(_score)?\\b","re.?assign.*index","re.?assign.*score","\\bri\\b"))
