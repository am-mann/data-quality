...
quality_vars <- c(
  "DQ_prop_garbage", "prop_light", "pct_overd_miss", "prop_all_comp"
)
...
res <- res %>%
  mutate(
    direction_score =
      ...
      z_prop_all_comp + z_DQ_prop_garbage)/6
  )
...
# ──────────────────────────────────────────────────────────────
quality_vars <- c("DQ_prop_garbage", "prop_light", "pct_overd_miss", "prop_all_comp")
...
for (pname in names(periods)) {
  ...
  peri_df <- peri_df %>%
    ...
    mutate(direction_score = (z_prop_light + z_pct_overd_miss -
                              z_prop_all_comp  + z_DQ_prop_garbage) / 4)
...
quality_vars <- c("prop_light", "pct_overd_miss", "prop_all_comp", "DQ_prop_garbage")
...
res <- df %>%
  ...
  mutate(direction_score =
           (z_prop_light + z_pct_overd_miss - z_prop_all_comp + z_DQ_prop_garbage) / 6)
