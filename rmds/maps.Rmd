...
vars_to_map <- c(
  ...
  "overdose_unspec_k", "marstat_comp_k", "placdth_comp_k",
  "educ_comp_k", "mandeath_comp_k", "age_comp_k",
  "sex_comp_k", "race_comp_k"
)
...
county_shapes_by_window <- map(shapefile_years, function(year) {
  ...
  if ("GEOID" %in% names(us_counties)) {
  ...
  } else if (all(c("STATE", "COUNTY") %in% names(us_counties))) {
    ...
    stop("Neither GEOID nor (STATE + COUNTY) available for this year")
  }
...
dq <- dq |> 
  mutate(
    ...
    countyrs_fips = if_else(
      year <= 2004,
      str_pad(countyrs, 5, pad = "0"),  # already FIPS for early years
      NA_character_
    ),
    state_abbr = if_else(year > 2004, substr(countyrs, 1, 2), NA_character_),
    county_part = if_else(year > 2004, substr(countyrs, 3, 5), NA_character_)
  ) |> 
  left_join(state_fips_lookup, by = "state_abbr") |> 
  mutate(
    countyrs_fips = if_else(
...
# ──────────────────────────────────────────────────────────────
cw_path   <- "/Users/amymann/Documents/Data Quality Project/data_quality/ihme_fips.rda"
...
# ──────────────────────────────────────────────────────────────
dq <- read_csv(
  "/Users/amymann/Documents/Data Quality Project/data_quality/county_year_quality_metrics.csv",
  show_col_types = FALSE
)
...
# ──────────────────────────────────────────────────────────────
# 3 · Robust FIPS parsing (handles NCHS 1999–2002 state codes)
# ──────────────────────────────────────────────────────────────
...
dq <- dq |>
  mutate(
    ...
    countyrs_fips = case_when(
      year <= 2002 ~ paste0(
        nchs_to_fips_state_lookup$fips_code[match(state_code, nchs_to_fips_state_lookup$nchs_code)],
        county_code
      ),
      TRUE ~ countyrs_chr  # already proper FIPS from 2003+
    )
  )
...
# ──────────────────────────────────────────────────────────────
unmatched <- dq |> filter(!countyrs_fips %in% ihme_xwalk$fips)

if (nrow(unmatched) > 0) {
  cat("⚠️  Warning: FIPS not matched to IHME crosswalk\n")
  print(unique(unmatched$countyrs_fips))
}
...
dq <- dq |>
 ...
 # ─────────────── Time-window-specific shapefiles with stable FIPS ───────────────
crs_proj <- 2163
...
county_shapes_by_window <- map(shapefile_years, function(year) {
  ...
  # Reproject and shift Alaska and Hawaii
  alaska <- us_counties |>
    filter(substr(stable_fips, 1, 2) == "02") |>
    st_scale(0.4) |>
    st_shift(c(1300000, -4900000)) |>
    st_set_crs(crs_proj)

  hawaii <- us_counties |>
    filter(substr(stable_fips, 1, 2) == "15") |>
    st_scale(1.5) |>
    st_shift(c(5200000, -1400000)) |>
    st_set_crs(crs_proj)

  mainland <- us_counties |>
    filter(!substr(stable_fips, 1, 2) %in% c("02", "15", "72")) |>
    st_set_crs(crs_proj)
...
# ──────────────────────────────────────────────────────────────
# 6 · Join shapefiles ↔ 5-year averages (by stable_fips)  (NEW key)
# ──────────────────────────────────────────────────────────────
joined_by_period <- map2(
  county_shapes_by_window, names(county_shapes_by_window),
  function(shapes, win) {
    dq_sub <- dq_avg |> filter(time_window == win)
    left_join(shapes, dq_sub, by = c("stable_fips"))
  }
) |> set_names(names(county_shapes_by_window))
...
} else if (var %in% c("pct_placdth_comp_k", "pct_educ_comp_k", "pct_marstat_comp_k")) {
  scale_fill_distiller(
    palette = palette,
    limits = c(99, 100),
    oob = scales::squish,
...
```{r}
################################################################################
## County size (n_cert) vs. every data-quality metric we mapped
## – ready to copy-paste
################################################################################
## 0 · Setup  ------------------------------------------------------------------
required <- c("tidyverse", "patchwork", "scales")
missing   <- setdiff(required, rownames(installed.packages()))
if (length(missing)) install.packages(missing)
invisible(lapply(required, library, character.only = TRUE))

# Suppose your county column is called "fips"
dq <- dq %>% rename(cnty = countyrs_fips)   # or GEOID, county_fips, etc.

## 1 · Convert all *_k completeness counts ➜ proportions  -----------------------
dq_plot <- dq %>%
  # turn every *_comp_k count into a proportion of certificates
  mutate(
    across(ends_with("_comp_k"),
           ~ .x / n_cert,
           .names = "{sub('_k$', '', .col)}_prop")
  )

## 2 · Pick the metrics you want on the Y-axis  ---------------------------------
# – garbage + lack-of-detail + every new *_prop column we just created
metrics <- c(
  "prop_garbage",      # underlying-cause garbage
  "prop_light",        # lightly coded (few total causes)
  "pct_acc_miss",      # accident missing detail (%)
  grep("_prop$", names(dq_plot), value = TRUE)  # all completeness proportions
) %>% unique()

## 3 · Tidy to long format  -----------------------------------------------------
county_id_cols <- intersect(c("cnty", "fips", "GEOID", "county_fips"), names(dq_plot))

dq_long <- dq_plot %>% 
  select(any_of(county_id_cols), n_cert, all_of(metrics)) %>% 
  pivot_longer(cols = all_of(metrics),
               names_to  = "metric",
               values_to = "value") %>% 
  drop_na(value, n_cert)

## 4 · Nice axis/plot labels  ---------------------------------------------------
label_lookup <- tibble(
  metric = metrics,
  label  = c(
    "Garbage-code proportion",
    "Lightly-coded proportion",
    "Accident detail missing (%)",
    stringr::str_replace_all(metrics[4:length(metrics)],
                             "_prop$", " completeness")
  )
)

dq_long <- dq_long %>% left_join(label_lookup, by = "metric")

## 5 · Build one scatter-plot per metric  --------------------------------------
plot_list <- dq_long %>%
  group_split(metric) %>%
  map(~ ggplot(.x, aes(n_cert, value)) +
        geom_point(alpha = 0.35, size = 1) +
        geom_smooth(method = "loess", se = FALSE, linewidth = 1) +
        scale_x_log10(labels = comma,
                      name   = "Death certificates in county-year") +
        scale_y_continuous(labels = percent_format(accuracy = 1)) +
        labs(y = NULL,
             title = unique(.x$label)) +
        theme_bw(base_size = 12))

## 6 · Show them side-by-side (2 columns)  -------------------------------------
wrap_plots(plotlist = plot_list, ncol = 2) +
  plot_annotation(
    title    = "County size vs. data-quality metrics",
    subtitle = "U.S. counties, 1999–2022 (each point = county-year)",
