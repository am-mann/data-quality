...
library(ggplot2)
library(scales)
library(tigris)       # for fips_codes
library(tidycensus)   # for county populations
library(readr)
library(here)
...
# 2 · Build FIPS lookup for post-2002 codes -------------------------------
fips_lookup <- fips_codes %>%
  mutate(
    county_code_3 = sprintf("%03d", as.integer(county_code)),  # e.g. "079"
    custom_code   = paste0(state, county_code_3),              # e.g. "ID079"
    GEOID         = paste0(state_code, county_code)            # e.g. "16079"
  ) %>%
  select(custom_code, GEOID)

# 3 · Convert all county codes to standard GEOID --------------------------
dq_all <- dq %>%
  mutate(
    GEOID = case_when(
      str_detect(countyrs, "^[0-9]{5}$") ~ countyrs,  # already a GEOID
      str_detect(countyrs, "^[A-Z]{2}[0-9]{3}$") ~ fips_lookup$GEOID[match(countyrs, fips_lookup$custom_code)],
      TRUE ~ NA_character_
    )
  )

# Check how many counties failed to match
...
county_sizes <- pop22 %>%
  ...
  select(GEOID, size_bucket)

# 6 · Join size buckets into main data ------------------------------------
# Join both population and bucket by using the original `pop22` data
dq_all <- dq_all %>%
  left_join(pop22, by = "GEOID") %>%
  left_join(county_sizes, by = "GEOID")


# 7 · Summarize time series by size bucket -------------------------------
ts_bucket <- dq_all %>%
  group_by(year, size_bucket) %>%
  summarise(avg_prop_garbage = mean(prop_garbage, na.rm = TRUE), .groups = "drop")

ts_bucket_clean <- ts_bucket %>%
  filter(!is.na(size_bucket))

# 8 · Plot time series ---------------------------------------------------
ggplot(ts_bucket_clean, aes(year, avg_prop_garbage, colour = size_bucket)) +
  geom_line(linewidth = 1) +
  scale_y_continuous(labels = percent_format(accuracy = 0.1)) +
  labs(
    title = "Garbage-code trend by county size",
    x = NULL,
    y = "Mean proportion garbage",
    colour = "2022 size bucket"
  ) +
  theme_bw()

# 9 · Save plot (optional) -----------------------------------------------
ggsave(here("figures", "garbage_trend_by_size_bucket.png"),
       width = 7, height = 4, dpi = 300)

# Filter for 2022 with valid population
scatter_data <- dq_all %>%
  filter(year == 2022, !is.na(pop_2022), !is.na(prop_garbage))

ggplot(scatter_data, aes(x = pop_2022, y = prop_garbage)) +
  geom_point(alpha = 0.6) +
  scale_x_log10(labels = scales::comma) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  labs(
    title = "County population vs. proportion garbage code (2022)",
    x = "County population (log10 scale)",
    y = "Proportion of garbage code"
  ) +
  theme_bw()

ggsave(here("figures", "scatter_population_vs_garbage_2022.png"),
       width = 7, height = 4, dpi = 300)
