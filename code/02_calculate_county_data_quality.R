...
.read_icd_map <- function(path) {
    ...
    if (any(grepl("^\\d+$", out$bin))) {
        ...
        stop(
            "USCOD normalized to digits-only bins (e.g. ", paste(head(bad, 8), collapse = ", "),
...
map_icd_to_bin <- function(x, lu) {
    ...
    m4 <- match(x4, lu$map4$icd4)
    ok4 <- !is.na(m4)
    out[ok4] <- lu$map4$bin4[m4[ok4]]
    m3 <- match(x3, lu$map3$icd3)
    ok3 <- !is.na(m3) & !ok4
    out[ok3] <- lu$map3$bin3[m3[ok3]]
    out
}
...
.shannon_H <- function(p) {
    p <- p[p > 0]
    if (!length(p)) 0 else -sum(p * log(p))
}
...
summarise_year_file <- function(file) {
    ...
    is_suicide_ucod <- function(uc4) {
        ...
        x <- suppressWarnings(as.integer(sub("^X", "", r3)))
    ...
    is_undet_drug_ucod <- function(uc4) {
        r3 <- toupper(substr(uc4, 1, 3))
        y <- suppressWarnings(as.integer(sub("^Y", "", r3)))
...
if (PARALLELIZE) {
    future::plan(future::multisession(workers = max(1, floor(future::availableCores() / 3))))
